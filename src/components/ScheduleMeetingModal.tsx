...
export const ScheduleMeetingModal = ({
  ...
  useEffect(() => {
    const fetchCurrentUserAvailability = async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('profiles').select('availability').eq('id', session.user.id).single();
    ...
    const fetchAISuggestions = async () => {
      ...
      try {
        const {
          data,
          error
        } = await supabase.functions.invoke('suggest-meeting-times', {
          body: {
  ...
  const calculateOverlappingSlots = (userAvail: WeekAvailability, recipientAvail: WeekAvailability): TimeSlot[] => {
    ...
    try {
      ...
      if (!meetingId) {
        const {
          ...
          error: checkError
        } = await supabase.from('meetings').select('*').or(`and(requester_id.eq.${session.user.id},recipient_id.eq.${recipientId},status.eq.pending),and(requester_id.eq.${recipientId},recipient_id.eq.${session.user.id},status.eq.pending)`);
      ...
      if (meetingId) {
        ...
        } = await supabase.from('profiles').select('email, google_calendar_connected').eq('id', recipientId).single();
  ...
  return <>
              ...
              <div className="p-4 space-y-4">
                <div className="text-center">
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    {meetingId ? 'Select a new time slot. Your partner will need to confirm the new time.' : 'Select a time slot that works for both of you'}
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input type="text" value={location} onChange={e => setLocation(e.target.value)} className="w-full px-3 py-1.5 text-sm rounded-full bg-white/5 border border-white/20 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-white/30" placeholder="Office, Coffee shop..." />
                  </div>

                  {loadingSlots ? <div className="text-center py-6">
                    ...
                    </div> : overlappingSlots.length === 0 ? <div className="text-center py-6">
                      ...
                      <div className="max-h-[300px] overflow-y-auto space-y-2 pr-1">
                        {overlappingSlots.map((slot, index) => <Button key={index} variant="outline" className="w-full justify-start h-auto py-2 px-3 rounded-2xl bg-white/5 border-white/20 text-white hover:bg-white/10 hover:border-white/30 transition-all" onClick={() => handleSlotClick(slot)} disabled={loading}>
                            <div className="flex items-center gap-2 w-full">
                              ...
                              <div className="flex-1 text-left min-w-0">
                                ...
                                {slot.displayTime && <div className="text-xs text-slate-400 mt-0.5 line-clamp-2 break-words">
                                    {slot.displayTime}
                                  </div>}
                              </div>
                            </div>
      ...
      {/* Confirmation Dialog */}
      {selectedSlot && <Dialog open={showConfirmation} onOpenChange={open => {
              ...
              <LiquidCrystalCard className="w-full">
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" className="flex-1 rounded-full bg-white/5 border-white/20 text-white hover:bg-white/10" onClick={() => {
                  setShowConfirmation(false);
                  setSelectedSlot(null);
                }}>
                      Cancel
                    </Button>
                    <Button className="flex-1 rounded-full bg-gradient-to-r from-cyan-500 to-blue-600 text-white hover:opacity-90" onClick={() => {
